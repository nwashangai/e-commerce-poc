...
const renderItem = (product, key) => (
  <div className="flex w-full justify-between" key={key}>
    <div className="w-2/3 flex flex-col py-1">
      <span className="text-xs text-bold pt-1">
        {toTitleCase(product.data.name)}
      </span>
      <span className="text-lg text-secondary pb-1">
        $ {product.data.price}
      </span>
    </div>
    <div className="w-1/3">
...
const Cart = ({ items, setIsCartOpen, clearCart }) => (
  <div className="absolute bg-pure right-1 border-2 border-line p-4 top-12 w-72 z-10">
    <div
      className="absolute top-1 -right-2 px-4 cursor-pointer"
      onClick={() => {
        setIsCartOpen(false);
      }}
    >
    ...
    <div className="mb-5 mt-6">
      {items.length > 0 ? (
        ...
        <div className="text-secondary">No item available</div>
      )}
    </div>
